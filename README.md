...
- debounceTime - default: 0 - debounce `onContentChanged`, `onEditorChanged`, `ngModel` and form control value changes. Can improve performance when working with large (>3 MiB) Deltas, as neither `editorChangeHandler`, nor `textChangeHandler` handler runs internally. Delays 
