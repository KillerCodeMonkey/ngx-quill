...
- onlyFormatEventData - flag if onContentEditorChanged/onEditorChanged only sets format fitting the editor format, default `false`, possible values `true | false | 'none'`. Performance boost when non `html` format is used (avoids expensive calls to `quill.getSemanticHtml()`). `none` skips _all_ extra format data, so the event only returns the `delta` and `oldDelta`.
