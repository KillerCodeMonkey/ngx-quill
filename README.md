...
- onlyFormatEventData - flag if onContentEditorChanged/onEditorChanged only sets format fitting the editor format, default `false`, possible values `true | false | 'none'`, performance boost when not html format is used (avoid additional callings of `quill.getSemanticHtml()`)
